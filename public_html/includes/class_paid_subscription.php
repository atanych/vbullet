...
* @date		$Date: 2008-11-07 05:41:04 -0600 (Fri, 07 Nov 2008) $
*
* @abstract
*
*/
	...
	*/
	function delete_user_subscription($subscriptionid, $userid, $subid = -1, $adminoption = false)
	{
		...
		if ($user['userid'] AND $sub['subscriptionid'])
		{
			...
			//access masks
			if (!empty($sub['forums']) AND @unserialize($sub['forums']))
			{
				$access_forums = array_keys(unserialize($sub['forums']));
				if (is_array($access_forums) AND !empty($access_forums))
				{
					$this->registry->db->query_write("
						DELETE FROM " . TABLE_PREFIX . "access
						WHERE forumid IN (" . implode(',', $access_forums)  . ") AND
							userid = $userid
					");
				}
			}
