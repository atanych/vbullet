...
require_once ( "./include/xmlrpcs.inc" );
$_POST['xmlrpc'] = 'true';
...
if(isset($_GET['method_name']) && in_array($_GET['method_name'], $tt_server_get_method)){
    include './include/function_server.php';
    $tt_server_function = $_GET['method_name'].'_func';
}

if (isset($tt_server_function) && function_exists($tt_server_function)){
    $tt_server_function();
    exit;
}
...
if(in_array($request_method, array('get_config', 'authorize_user', 'login', 'sign_in', 'register', 'prefetch_account', 'get_contact')))
{
...
if($tt_config['guest_okay'] == 0 && $vbulletin->userinfo['userid'] == 0 && $request_method != 'get_contact'&& $request_method != 'get_config' && $request_method != 'login' && $request_method != 'register' && $request_method != 'sign_in' && $request_method != 'prefetch_account')
    return_fault();
