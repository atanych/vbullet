...
{
    global $vbulletin, $show, $lastpostarray, $can_subscribe;
    ...
    foreach ($vbulletin->iforumcache["$parentid"] AS $forumid)
    {
        ...
        if (!$subsonly OR ($subsonly AND !empty($forum['subscribeforumid'])))
        {
            ...
            $mobiquo_can_subscribe = $can_subscribe && ($forum['options'] & $vbulletin->bf_misc_forumoptions['allowposting']) && !$forum['link'] && !$only_sub;
            ...
            if ($newPost)   $forumbits_list['new_post']         = new xmlrpcval(true, 'boolean');
            ...
            if ($logo_url = get_forum_icon($forumid, $forum['depth'] === 0 ?'category':($forum['link']?'link':'forum'), $forum['depth'] !== 0 && $forum_locked ? true : false, $newPost))
