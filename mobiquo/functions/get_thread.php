...
{
    global $db, $vbulletin, $permissions, $vbphrase, $html_content, $forum, $show, $postids;
    ...
    else if(isset($vbulletin->options['vbseo_like_enable']) && $vbulletin->options['vbseo_like_enable'])
    {
    ...
    while ($post = $db->fetch_array($posts))
    {
        ...
        if($html_content)
        {
            ...
            $post_content = preg_replace("/\[\/spoiler\]/siU", '[/spoiler1]', $post_content);
            $post_content = preg_replace('/\[url="(.*?)"\]/siU', "[url=$1]", $post_content);
            $bbcode_parser = new vB_BbCodeParser($vbulletin, $a, false);
            $post_content = $bbcode_parser->parse( $post_content, $thread['forumid'], false);
            $post_content = preg_replace("/\[\/spoiler1\]/siU", '[/spoiler]', $post_content);
        ...
        $return_post = array(
            ...
            'user_type'         => new xmlrpcval(get_usertype_by_name(mobiquo_encode($post['postusername'])), 'base64'),
        ...
        if(file_exists(DIR ."/mobiquo/debug.on") && isset($_SERVER['HTTP_DEBUG']) && $_SERVER['HTTP_DEBUG'])
        {
            if(isset($vbulletin->products['dbtech_thanks']) && $vbulletin->products['dbtech_thanks'])
                header("hasdbtech:true");
            if(isset($vbulletin->options['dbtech_thanks_features']) && $vbulletin->options['dbtech_thanks_features'])
                header("dbtechoption:".$vbulletin->options['dbtech_thanks_features']);
        }
        if(isset($vbulletin->options['dbtech_thanks_features']) && $vbulletin->options['dbtech_thanks_features'] && isset($vbulletin->products['dbtech_thanks']) && $vbulletin->products['dbtech_thanks'])
        {
            // Add dbTech Support
            ...
            if ($like_count)
            {
                ...
                foreach($alikes as $like)
                {
                    $like_list[] = new xmlrpcval(array(
                        ...
                        'user_type' => new xmlrpcval(get_usertype_by_name(mobiquo_encode($like['l_from_username'])), 'base64'),
                    ), 'struct');
                }
            ...
            if (!empty($thanks))
            {
                ...
                foreach ($thanks as $thank)
                {
                    $thank_list[] = new xmlrpcval(array(
                        ...
                        'user_type' => new xmlrpcval(get_usertype_by_name(mobiquo_encode($thank['username'])), 'base64'),
                    ), 'struct');
                }
    ...
    if (!$vbulletin->userinfo['userid']
        ...
        OR !$foruminfo['allowposting'] OR $foruminfo['link'] OR !$foruminfo['cancontainthreads'])
    {
        $can_subscribe = false;
    }
    ...
    $return_data = array(
        ...
        'topic_author_avatar' => new xmlrpcval($icon_url, 'string'),
    );
