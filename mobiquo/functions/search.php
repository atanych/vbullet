...
{
    $_REQUEST['userid'] = $vbulletin->session->userinfo['userid'];
}
...
//	}
	...
	if ($errors)
	{
	    return_fault(fetch_error($errors[0][0], $errors[0][1]));
    ...
    $vbulletin->GPC['searchid'] = $results->get_searchid();
    $_REQUEST['do'] = 'showresults';
}
...
if ($_REQUEST['do'] == 'getnew' OR $_REQUEST['do'] == 'getdaily')
{
	...
	if ($errors)
	{
		return_fault(fetch_error($errors[0]));
	}
...
if ($_REQUEST['do'] == 'finduser')
{
	...
	// valid user id?
	if (!$vbulletin->GPC_exists['userid'] and !$vbulletin->GPC_exists['postuserid'] and !$vbulletin->GPC['searchuser'])
	{
	    return_fault(fetch_error(fetch_error('invalidid', $vbphrase['user'], $vbulletin->options['contactuslink'])));
	}
	...
	else
	{
		if ($vbulletin->GPC['include'])
		{
			...
			if (is_array($list))
			{
				...
				$criteria->add_forumid_filter($list, true);
			}
		}

		if ($vbulletin->GPC['exclude'] && $vbulletin->GPC['exclude'] != -1)
		{
			...
			{
				$list = array_map('intval', $list);
				$list = fetch_search_forumids($list, 1);
