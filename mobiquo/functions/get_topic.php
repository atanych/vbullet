...
{
    global $vbulletin, $permissions, $show, $db, $vbphrase, $newthreads, $dotthreads, $perpage, $ignore;
    ...
    if (isset($vbulletin->options['tapatalk_readonly_forums']))
    {
        ...
        if (is_array($read_only_forums))
            $is_read_only_forums = in_array($vbulletin->GPC['forumid'], $read_only_forums);
    }
    ...
    $show['is_subscribed'] = ($vbulletin->forumcache["$foruminfo[forumid]"]['subscribeforumid'] != '' ? true : false);
        ...
        if($api_mode == 'ANN')
        {
            ...
            while ($announcement = $db->fetch_array($announcements))
            {
                ...
                $return_post = new xmlrpcval(array( 'forum_id'=> new xmlrpcval($thread['forumid'], 'string'),
                    ...
                    'user_type'         => new xmlrpcval(get_usertype_by_name(mobiquo_encode($announcement['username'])), 'base64'),
...
//            }
            ...
            while ($thread = $db->fetch_array($threads))
            {
                ...
                if ($thread['visible'] != 2 || can_moderate($forumid))
                {
                    ...
                    $return_post = array(
                        ...
                        'user_type'         => new xmlrpcval(get_usertype_by_name(mobiquo_encode($thread['postusername'])), 'base64'),
    ...
    if ($show['is_subscribed'])  $return_data['is_subscribed']  = new xmlrpcval(true, 'boolean');
    ...
    if ($show['movethread'])     $return_data['can_merge']      = new xmlrpcval(true, 'boolean');
     
    
    $return_data['topics']  = new xmlrpcval($return_thread, 'array');
