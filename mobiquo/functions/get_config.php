...
function get_config_func()
{
    ...
    if(!$isTTServerCall)
    {
        $version_arr = explode('_', $tt_config['version']);
        $return_config['version'] =  new xmlrpcval($version_arr[0],'string');
        unset($return_config['sys_version']);
        unset($return_config['stats']);
    }
