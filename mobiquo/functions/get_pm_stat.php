        ...
        foreach ($pms AS $pmid => $pm)
        {
            ...
            else
            {
                ...
                if ($folderid == -1)
                {
                    ...
                    foreach ($touser AS $userid => $username)
                    {
                        ...
                        $return_to_user = new xmlrpcval(array(
                            ...
                            'user_type'    => new xmlrpcval(get_usertype_by_name(mobiquo_encode($username)), 'base64'),
                        ), 'struct');
                ...
                $return_message = array(
                    ...
                    'icon_url'      => new xmlrpcval(mobiquo_get_user_icon2($icon_user_id), 'string'),
            ...
            foreach($item AS $subkey => $subitem)
            {
                if ($key != 'bcc' || $pm['fromuserid'] == $vbulletin->userinfo['userid'] || $subkey == $vbulletin->userinfo['userid'])
                {
                    ...
                    $return_to_user = new xmlrpcval(array(
                        ...
                        'user_type' => new xmlrpcval(get_usertype_by_name(mobiquo_encode($subitem)), 'base64'),
                    ), 'struct');
                    array_push($return_to_users, $return_to_user);
                }
            }
        ...
        else
        {
            if ($pm['fromuserid'] == $vbulletin->userinfo['userid'] || $key == $vbulletin->userinfo['userid'])
            {
                $return_to_user = new xmlrpcval(array(
                    ...
                    'user_type' => new xmlrpcval(get_usertype_by_name(mobiquo_encode($item)), 'base64'),
                ), 'struct');
                array_push($return_to_users, $return_to_user);
            }
        }
...
function create_message_func($xmlrpc_params)
{
    ...
    $vbulletin->GPC['message'] = $params[2];//mobiquo_encode($params[2], 'to_local');
    ...
    $pm['message'] = mobiquo_encode($params[2], 'to_local');
...
function mark_pm_read_func($xmlrpc_params)
{
    ...
    else if (strpos($messageid, 'textid_') === 0)
    {
        $pmtextid = intval(substr($messageid, 7));
        $pm_where = " AND pmtextid='$pmtextid' ";
    }
